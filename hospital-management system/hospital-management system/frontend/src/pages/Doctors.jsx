  ...
  return (
    <>
      <div className="flex flex-wrap justify-between items-start gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-teal-900">Doctors</h1>
          <p className="text-teal-700 text-sm mt-1">Manage doctors and specializations</p>
        </div>
              ...
              <tbody>
                {doctors.map((d) => (
                  <tr key={d._id} className="hover:bg-teal-50/50">
                    <td className="px-4 py-3 border-b border-teal-100">{d.name}</td>
                    <td className="px-4 py-3 border-b border-teal-100">{d.email}</td>
                    <td className="px-4 py-3 border-b border-teal-100">{d.specialization}</td>
                    <td className="px-4 py-3 border-b border-teal-100">{d.department?.name || '-'}</td>
                    <td className="px-4 py-3 border-b border-teal-100">{d.consultationFee ? `₹${d.consultationFee}` : '-'}</td>
      ...
      {modal && (
        <div className="fixed inset-0 bg-teal-900/40 flex items-center justify-center z-50 p-4" onClick={() => setModal(null)}>
          <div className="bg-white rounded-xl shadow-xl max-w-[520px] w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="p-5 border-b border-teal-200 flex justify-between items-center">
              <h2 className="text-xl font-bold text-teal-900">{modal === 'add' ? 'Add Doctor' : 'Edit Doctor'}</h2>
              ...
              <div className="p-5">
                ...
                <div className={formGroup}>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {days.map((day) => (
                      <button key={day} type="button" className={`py-1.5 px-2.5 rounded-lg text-sm font-semibold ${form.availableDays.includes(day) ? 'bg-teal-500 text-white' : 'border border-teal-200 bg-teal-50 text-teal-900 hover:bg-teal-100'}`} onClick={() => toggleDay(day)}>{day}</button>
                    ))}
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
