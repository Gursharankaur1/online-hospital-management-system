  ...
  return (
    <>
      <div className="flex flex-wrap justify-between items-start gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-teal-900">Medical Records</h1>
          <p className="text-teal-700 text-sm mt-1">Patient diagnosis, prescriptions and vitals</p>
        </div>
        ...
        <div className="overflow-x-auto rounded-lg border border-teal-200">
          {loading ? (
            ...
            <table className="w-full border-collapse text-sm">
              ...
              <tbody>
                {records.map((r) => (
                  <tr key={r._id} className="hover:bg-teal-50/50">
                    <td className="px-4 py-3 border-b border-teal-100">{r.patient?.name}</td>
                    <td className="px-4 py-3 border-b border-teal-100">{r.doctor?.name} ({r.doctor?.specialization})</td>
                    <td className="px-4 py-3 border-b border-teal-100 max-w-[220px]">{r.diagnosis ? (r.diagnosis.length > 40 ? r.diagnosis.slice(0, 40) + '…' : r.diagnosis) : '-'}</td>
                    <td className="px-4 py-3 border-b border-teal-100">{new Date(r.createdAt).toLocaleDateString()}</td>
                    <td className="px-4 py-3 border-b border-teal-100">
                      <button type="button" className="py-1.5 px-3 rounded-lg font-medium text-sm border border-teal-200 bg-teal-50 text-teal-900 hover:bg-teal-100 mr-2" onClick={() => openEdit(r)}>Edit</button>
      ...
      {modal && (
        <div className="fixed inset-0 bg-teal-900/40 flex items-center justify-center z-50 p-4" onClick={() => setModal(null)}>
          <div className="bg-white rounded-xl shadow-xl max-w-[560px] w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="p-5 border-b border-teal-200 flex justify-between items-center">
              <h2 className="text-xl font-bold text-teal-900">{modal === 'add' ? 'Add Medical Record' : 'Edit Medical Record'}</h2>
                ...
                <div className={formGroup}>
                  ...
                  {form.prescription.map((p, i) => (
                    <div key={i} className="grid grid-cols-[1fr_1fr_1fr_auto] gap-2 items-end mb-2">
                      ...
                      <button type="button" className="py-2 px-2 rounded-lg border border-teal-200 bg-teal-50 text-teal-900 hover:bg-teal-100 disabled:opacity-50" onClick={() => removePrescriptionLine(i)} disabled={form.prescription.length <= 1}>−</button>
                    </div>
                  ))}
                  <button type="button" className="py-2 px-3 rounded-lg font-medium text-sm border border-teal-200 bg-teal-50 text-teal-900 hover:bg-teal-100 mt-1" onClick={addPrescriptionLine}>+ Add line</button>
                </div>
                <div className={formGroup}>
